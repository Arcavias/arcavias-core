...
/**
 * Test class for MShop_Price_Manager_Unit_Default.
 */
class MShop_Price_Manager_Unit_DefaultTest extends MW_Unittest_Testcase
{
	private $_object;
	private $_editor = '';
	...
	protected function setUp()
	{
		...
		$this->_object = $manager->getSubManager( 'unit' );
	}
	...
	{
		$this->assertInstanceOf( 'MShop_Price_Item_Unit_Interface', $this->_object->createItem() );
	}
	...
	public function testSaveUpdateDeleteItem()
	{
		...
		$search->setConditions( $search->compare( '==', 'price.unit.editor', $this->_editor ) );
	...
	public function testGetItem()
	{
		$search = $this->_object->createSearch();
		$conditions = array(
			$search->compare( '==', 'price.unit.code', 'default' ),
			$search->compare( '==', 'price.unit.editor', $this->_editor )
		);
		$search->setConditions( $search->combine( '&&', $conditions ) );
		$result = $this->_object->searchItems( $search );

		if( ($expected = reset($result)) === false ) {
			throw new Exception( sprintf( 'No price unit item found for code "%1$s"', 'default' ) );
		}
	...
	public function testSearchItems()
	{
		...
		$search = $this->_object->createSearch();
		$conditions = array(
			$search->compare( '~=', 'price.unit.code', 'unit' ),
			$search->compare( '==', 'price.unit.editor', $this->_editor )
		);
		$search->setConditions( $search->combine( '&&', $conditions ) );
		$search->setSlice( 0, 2 );
